...
app.config["SQLALCHEMY_BINDS"] = {
    "db3": "mysql+pymysql://root:mysql@192.168.243.154:3306/db3",
    "db4": "mysql+pymysql://root:mysql@192.168.243.154:3306/db4",
}
...
# 创建数据库对象
# 设置分布式事务：二阶段提交事务
# 参数：session_options={"twophase": True}
...
def hello_world():
    ...
    # 虽然代码层面只有一次commit提交操作，开启了分布式事务-保证事务原子性
    # 二阶段提交：1.事务管理器预提交 2.所有预提交成功-commit 3.只有有一个预提交失败，整体回滚-rollback
    db.session.commit()
    return 'Hello World!'
